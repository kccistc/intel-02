...
model_path = Path(f"mosaic-9.onnx")

style_url = f"{base_url}/{model_path}"
utils.download_file(style_url, directory=base_model_dir)

ov_model = ov.convert_model(f"model/mosaic-9.onnx")
ov.save_model(ov_model, f"model/mosaic-9.xml")

# Converted IR model path
ir_path = Path(f"model/mosaic-9.xml")
