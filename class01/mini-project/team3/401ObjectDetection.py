    ...
    try:
        ...
        while True:
            ...
            if use_popup:
                ...
                if key == 27:
                    for label, score, box in boxes:
                    	if label == 8 or label == 3 or label == 6:#label 8 is truck
                    	    flag = 1
                    	    tl_x = box[0]
                    	    tl_y = box[1]
                    	    br_x = box[0] + box[2]
                    	    br_y = box[1] + box[3]
                    	    #print(str(tl_x)+", "+str(tl_y)+", "+str(br_x)+", "+str(br_y))
                    	    cropped = frame[tl_y:br_y, tl_x:br_x]
                    	    cv2.imwrite("truck.jpg", cropped)
                    if flag:break
                # Crop truck, car, or bus from original image
                '''
                elif key == ord('c'):
                    for label, score, box in boxes:
                    	if label == 8 or label == 3 or label == 6:#label 8 is truck
